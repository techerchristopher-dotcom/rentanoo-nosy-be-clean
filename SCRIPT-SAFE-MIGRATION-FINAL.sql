...
-- ============================================================================
-- ÉTAPE 2: CRÉATION DES FONCTIONS CUSTOM
-- ============================================================================
...
-- ============================================================================
-- ÉTAPE 6: CRÉATION IDEMPOTENTE DES TRIGGERS
-- ============================================================================

-- Trigger INSERT sur profiles (création idempotente)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 
        FROM pg_trigger 
        WHERE tgname = 'trigger_sync_profile_is_admin_insert'
        AND tgrelid = 'public.profiles'::regclass
    ) THEN
        CREATE TRIGGER trigger_sync_profile_is_admin_insert
            AFTER INSERT ON public.profiles
            FOR EACH ROW
            EXECUTE FUNCTION public.sync_profile_is_admin();
    END IF;
END $$;

-- Trigger UPDATE sur profiles (création idempotente)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 
        FROM pg_trigger 
        WHERE tgname = 'trigger_sync_profile_is_admin_update'
        AND tgrelid = 'public.profiles'::regclass
    ) THEN
